...
class AdminModel {
  ...
  async findReportById(reportId) {
    ...
    const [wordResult, reporterResult] = await Promise.all([
      supabase.from('vocabulary').select('*, creator:users(id, display_name)').eq('id', report.word_id).single(),
      supabase.from('users').select('id, display_name, email').eq('id', report.reporter_id).single()
    ]);
