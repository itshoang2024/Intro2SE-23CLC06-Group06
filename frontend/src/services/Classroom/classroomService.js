...
const classroomService = {
  ...
  approveJoinRequest: async ({ classroomId, learnerId }) => {
    // chua cap nhat data ben ApproveClassroomRequest
    const res = await api.post(`/classroom/${classroomId}/approve-request`, {
      learnerId,
    });
    return res.data;
  },
  ...
  rejectJoinRequest: async ({ classroomId, learnerId }) => {
    const res = await api.post(`/classroom/${classroomId}/reject-request`, {
      learnerId,
    });
    return res.data;
  },
  ...
  removeALearner: async ({ classroomId, learnerId }) => {
    const res = await api.post(`/classroom/${classroomId}/remove-learner`, {
      learnerId,
    });
    return res.data;
  },
  ...
  inviteLearner: async ({ classroomId, email }) => {
    const res = await api.post(`/classroom/${classroomId}/invitation`, {
      email,
    });
    return res.data;
  },
  ...
  getToReviewAssignments: async (classroomId) => {
    const res = await api.get(
      `/classroom/${classroomId}/assignments/to-review`
    );
    return res.data;
  },
  ...
  changeAutoApproveSetting: async (classroomId, { isAutoApprovalEnabled }) => {
    const res = await api.patch(`/classroom/${classroomId}/auto-approve`, {
