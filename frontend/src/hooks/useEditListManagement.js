...
export const useEditListManagement = (listId, validationHook, wordManagementHook) => {
  ...
  const { getValidWords, setWords, setOriginalWords, words, getWordsForUpdate } = wordManagementHook;
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    console.log('Words to validate (existing + new):', {
      ...
      existing: nonEmptyWordsWithIndex.filter(item => item.isExisting).length,
    ...
    const normalizedWordsWithIndex = nonEmptyWordsWithIndex.map((item) => {
      ...
      console.log(`Word at position ${originalIndex + 1} (${isExisting ? 'EXISTING' : 'NEW'}) normalized:`, {
        original: word,
    ...
    const normalizedWords = normalizedWordsWithIndex.map(({ _originalIndex, _isExisting, _isNew, ...word }) => word);
    
    console.log('Normalized words for validation (ALL non-empty words):', normalizedWords);
    ...
    if (validation.errors.words) {
      ...
      Object.keys(validation.errors.words).forEach(validationIndex => {
        ...
        const isExisting = normalizedWordsWithIndex[parseInt(validationIndex)]._isExisting;
        const isNew = normalizedWordsWithIndex[parseInt(validationIndex)]._isNew;
        
        console.log(`Mapping validation error for word at validation index ${validationIndex} -> original index ${originalIndex} (${isExisting ? 'EXISTING' : 'NEW'})`);
    ...
    console.log('Validation result with remapped indices (ALL words validated):', validation);
    ...
    console.log('Validation passed for ALL words, proceeding with submission...');
    ...
    try {
      ...
      const originalWordIds = new Set(originalWords.map(w => w.id).filter(id => id));
      const currentWordIds = new Set(validatedCompleteWords.filter(w => w.id).map(w => w.id));
      const deletedWordIds = [...originalWordIds].filter(id => !currentWordIds.has(id));

      console.log('Words to delete (removed existing words):', deletedWordIds);
      ...
      for (const deletedId of deletedWordIds) {
        try {
          ...
          console.error(`Failed to delete word ID ${deletedId}:`, deleteError);
      ...
      for (const word of validatedCompleteWords) {
        ...
        try {
          if (word.id && word._isExisting) {
            ...
            console.log(`Created new word with returned data:`, newWord?.data);
            createdCount++;
          }
        } catch (wordError) {
      ...
      console.log(`Word processing completed: ${updatedCount} updated, ${createdCount} created`);

      // Fetch updated words from server to sync state
      try {
        const updatedWordsList = await vocabularyService.getWordsByListId(listId);
        ...
        if (updatedWordsList && updatedWordsList.length > 0) {
          ...
          console.log('Synced updated words from server:', mappedUpdatedWords);
        }
      } catch (syncError) {
