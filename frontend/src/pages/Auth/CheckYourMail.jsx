...
export default function CheckYourMail() {
  ...
  useEffect(() => {
    ...
    const userEmail =
      location.state?.email || authService.getCurrentUser()?.email || "";
    setEmail(userEmail);
  }, [location.state]);
  ...
  return (
    <div
      className="check-your-mail-container"
      style={{
    ...
    >
      ...
      <div className="resend-section">
        ...
        {resendMessage && <div className="resend-success">{resendMessage}</div>}

        {resendError && <div className="resend-error">{resendError}</div>}
      </div>

      <Link to="/signin" className="go-to-sign-in">
        {" "}
        Go to sign in{" "}
      </Link>
      <img
